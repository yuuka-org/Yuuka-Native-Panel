...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    redirect('/wp_manager_themes?id=' . $id);
}
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <div class="btn-group">
    ...
    <a href="/wp_manager_themes?id=<?= (int) $id ?>" class="btn btn-primary">Tema</a>
  </div>
</div>
    ...
    <table class="table table-hover mb-0 align-middle">
      ...
      <?php foreach ($themes as $t): $rowId = md5($t['folder']); ?>
        <tr>
          ...
          <td class="text-end">
            <a href="/wp_manager_themes?id=<?= (int) $id ?>&check=<?= urlencode($t['folder']) ?>" class="btn btn-sm btn-outline-secondary" title="Cek update"><i class="bi bi-arrow-repeat"></i></a>
            <?php if (Rbac::can($user['role'], 'wp.manage')): ?>
              <?php if (!$t['active']): ?>
